...
onready var links = $Chainlinks	# A slightly easier reference to the links
var direction := Vector2.ZERO		# The direction in which the chain was shot
var tip := Vector2.ZERO			# The global position the tip should be in
...
func _physics_process(_delta: float) -> void:
	...
	if hooked:
		$Tip.position.x += Global.speed
		if not $Tip.move_and_collide(direction*SPEED):
			release()
